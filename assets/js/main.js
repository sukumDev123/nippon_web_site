  ...
  if (shade[shadeId]) {
    if (shade[shadeId].length) {
      ...
      Array.from(shade[shadeId]).forEach(function (shade, index) {
        ...
        if (index === 0) document.querySelector("#family-image").style.backgroundColor = color;
    ...
    if (_familyDiv.firstChild) {
      while (_familyDiv.lastElementChild) {
        ...
        document.querySelector("#family-image").style.backgroundColor = "rgba(0,0,0,0)";
      }
    }
...
function findColorClose(value, type, familyColors) {
  ...
  if (type === "rgb") {
    ...
    var suggestion1 = document.querySelector(".suggestion1 > .card-color-family > #bk-family-color");
    var titleFamilyColor = document.querySelector(".suggestion1 .card-color-family #title-family-color"); // console.log({ suggestion1 });
    ...
    _4Data.forEach(function (sort) {
      ...
      _cardFamilyColor.innerHTML = "\n        <div id=\"bk-family-color\" style=\"background-color:".concat(sort.color, "\"></div>\n        <h2 id=\"title-family-color\">\n          ").concat(sort.name, "\n        </h2>\n        \n        ");
...
function loadSolutionFromPage(id) {
  ...
  if (solution_div) {
    ...
    fetch(domain + "wp-json/wp/v2/solutions?_embed&page_parent=" + id).then(function (d) {
      return d.json();
    }).then(function (d) {
...
function homePageInitSwiper() {
  ...
  var product_owner = new Swiper(".products-card-owner-develop-swiper", {
    slidesPerView: 1
  });
    ...
    try {
      ...
      if (element && home_header) {
        if (element.offsetTop <= _scrollTop) {
          ...
          var ddd = home_header.className.split(" ").filter(function (c) {
...
var onClickedHeader = function onClickedHeader() {
  ...
  if ((_header$className$mat = header.className.match("header-active")) !== null && _header$className$mat !== void 0 && _header$className$mat.length) {
    var _ddd = header.className.split(" ").filter(function (c) {
    ...
    Array.from(menuTop).forEach(function (_menu) {
      ...
      if ((_menu$className$match = _menu.className.match("menu-active")) !== null && _menu$className$match !== void 0 && _menu$className$match.length) {
        var ddd = _menu.className.split(" ").filter(function (c) {
        ...
        if (element.offsetTop > scrollTop) {
          var _ddd2 = header.className.split(" ").filter(function (c) {
            return c != "home_header_fixed";
          }).join(" ");
      ...
      menu.querySelector("a").addEventListener("click", function (event) {
        ...
        if (menu.querySelector("a").href.match(/#/)) {
          ...
          Array.from(menuTop).forEach(function (_menu) {
            ...
            if ((_menu$className$match2 = _menu.className.match("menu-active")) !== null && _menu$className$match2 !== void 0 && _menu$className$match2.length) {
              var ddd = _menu.className.split(" ").filter(function (c) {
                return c != "menu-active";
              }).join(" ");
          ...
          if ((_menu$className$match3 = menu.className.match("menu-active")) !== null && _menu$className$match3 !== void 0 && _menu$className$match3.length) {
            var ddd = menu.className.split(" ").filter(function (c) {
              return c != "menu-active";
            }).join(" ");
  ...
  if (search_selected) {
    document.querySelector(".select-input").addEventListener("click", function () {
      if (search_selected.className.match(/selected-active/)) {
        var _className = search_selected.className.split(" ").filter(function (c) {
          return c !== "selected-active";
        }).join(" "); // console.log({ _className });


        search_selected.className = _className;
        document.querySelector("#selected_shade_color").className = "select-custom";
      } else {
        search_selected.className += " selected-active";
        document.querySelector("#selected_shade_color").className = "select-custom active-ul";
      }
    });
  }
};
...
var onPageShadeInit = function onPageShadeInit() {
  ...
  if (shade_filter) shade_filter.addEventListener("change", function (event) {
  ...
  if (show_video_link) {
    show_video_link.addEventListener("click", function () {
      if (show_video_link.className.match("show").length) {
        var _className = show_video_link.className.split(" ").filter(function (c) {
          return c != "show";
        });
...
var onSelectedFilterOnClick = function onSelectedFilterOnClick() {
  var product_filter_input = document.querySelector("#product_filter .select-input #show_product");
  var cate_filter_input = document.querySelector("#categories_project_filter .select-input input");
  var product_filter = document.querySelector("#product_filter ul");
  var categories_project_filter = document.querySelector("#categories_project_filter ul");
...
var select_product_id = function select_product_id(name, id) {
  ...
  var _product_value_ = document.querySelector("input[name=filter_product]");

  _product_value.value = name;
  _product_value_.value = id;
  product_filter.className = "";
  document.querySelector(".pfi").className = "fas fa-chevron-down pfi"; // window.location.href = "?filter_product=" + id;
};
...
var select_project_name = function select_project_name(name, id) {
  ...
  _product_value_.value = id;
  categories_project_filter.className = "";
  document.querySelector(".cgi").className = "fas fa-chevron-down cgi"; // window.location.href = "?filter_product=" + id;
};

var handleHeaderMobile = function handleHeaderMobile() {
  var header_mobile_close = document.querySelector("#header_mobile_close");
  ...
  if (show_menus_mobile) {
    show_menus_mobile.addEventListener("click", function (event) {
      if (!header_mobiles.className) header_mobiles.className = "active";else header_mobiles.className = "";
    });
  }
...
var handleMenuClickedShowSubMenu = function handleMenuClickedShowSubMenu() {
  var menu_top_menu_online = document.querySelectorAll("#menu-top-menu-online > li");
      ...
      menu.addEventListener("click", function (event) {
        ...
        if (menu.children[1]) {
          ...
          if (menu.children[1].className.match(/active/)) {
            menu.children[1].className = menu.children[1].className.split(" ").filter(function (m) {
              return m !== "active";
            }).join(" ");
          } else {
...
function handleFilterProductMobile() {
  var filter_product_close = document.querySelector("#filter_product_close");
  var filter_product_mobiles = document.querySelector("#filter_product_mobiles");
  var product_cate = document.querySelectorAll(".filter-content .product-cate-card");
...
function handleFooterMenuClicked() {
  var footer_menu_mobile = document.querySelectorAll(".footer-menu-mobile > div > ul > li");
    ...
    Array.from(footer_menu_mobile).forEach(function (ele) {
      ...
      if (ele.children[0]) {
        ele.children[0].addEventListener("click", function (event) {
          event.preventDefault();
          if (ele.className.match("active")) ele.className = ele.className.split(" ").filter(function (c) {
            return c != "active";
          }).join(" ");else ele.className += " active";
        });
      }
    });
  }
...
function solutionChange(id) {
  ...
  if (solution_div_array !== null && solution_div_array !== void 0 && solution_div_array.length) {
    for (var i = 0; i < solution_div_array.length; i++) {
      var _sDiv$className$match, _sDiv$id$match;

      var sDiv = solution_div_array[i];
      if ((_sDiv$className$match = sDiv.className.match("card-active")) !== null && _sDiv$className$match !== void 0 && _sDiv$className$match.length) sDiv.className = "";
      if ((_sDiv$id$match = sDiv.id.match("solution" + id)) !== null && _sDiv$id$match !== void 0 && _sDiv$id$match.length) sDiv.className = "card-active";
    }
  }

  fetch(domain + "/wp-json/wp/v2/solutions/" + id).then(function (data) {
    // console.log({ data });
    return data.json();
  }).then(function (data) {
    console.log({
    ...
    if (data.acf) {
      ...
      if (problem.after_image) {
        var before_image_problem = document.querySelector("#before_image_problem");
        before_image_problem.src = problem.before_image.url;
        var after_image_problem = document.querySelector("#after_image_problem");
      ...
      if (problem.title) {
        ...
        var problem_sub_title = document.querySelector("#problem_sub_title");
        problem_sub_title.innerHTML = problem.sub_title;
        var problem_result = document.querySelector("#problem_result");
  ...
  if (provinceElement && districtElement && countryElement) {
    countryElement.addEventListener("change", function (event) {
      ...
      if (value.toUpperCase() === "TH") {
        Promise.all([fetch("https://staging.tanpong.me/wp-content/themes/nippontheme/assets/json/province.json"), fetch("https://staging.tanpong.me/wp-content/themes/nippontheme/assets/json/districts.json")]).then(function (d) {
    ...
    provinceElement.addEventListener("change", function (event) {
      ...
      while (districtElement.lastElementChild) {
        if (districtElement.lastElementChild.value == "" || districtElement.lastElementChild.value == "เมือง") break;
    ...
    document.querySelector("#location_search").addEventListener("click", function (event) {
      ...
      if (catProductValue) {
        ...
        if (provinceValue) {
          ...
          search_all = districtValue + " " + province[provinceValue].PROVINCE_NAME;
        }
      } else {
        // search +=
        //   "?country=" +
...
/******/ 	__webpack_require__.O(undefined, ["assets/css/main"], () => (__webpack_require__("./src/app.js")))
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["assets/css/main"], () => (__webpack_require__("./src/main.scss")))
